...
def worker_process(gpu_id: int, tasks_chunk: list, args, return_dict: dict):
    ...
    try:
        ...
        for task in tasks_chunk:
            ...
            if temp_path:
                batch_prompts.append({"image_path": temp_path, "text_prompt": task["question"], "task_info": task})

        # Run batch prediction
        predictions = predictor.predict_batch(batch_prompts, temperature=args.temperature, max_tokens=args.max_tokens)
        ...
        for prompt, prediction in zip(batch_prompts, predictions):
            result = {"gpu_id": gpu_id, "task_info": prompt["task_info"], "prediction": prediction.strip()}
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="OCR evaluation script with vLLM", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )

    # Dataset configuration
    parser.add_argument("--shard-path", type=str, default="shard-00000.tar", help="Path to the tar shard dataset file")
    parser.add_argument("--limit", type=int, default=5, help="Number of samples to evaluate (0 for all)")

    # Model configuration
    parser.add_argument(
        "--model-name", type=str, default="Qwen/Qwen2.5-VL-7B-Instruct", help="Name of the model to evaluate"
    )
    parser.add_argument("--system-prompt", type=str, default="system_prompt.txt", help="Path to system prompt file")
    ...
    parser.add_argument(
        "--num-workers", type=int, default=1, help="Number of parallel worker processes (data parallelism)"
    )
    ...
    parser.add_argument(
        "--input-types", nargs="+", default=None, help="Input types to restrict tasks (e.g., 'image', 'image+text')"
    )
    parser.add_argument(
        "--output-types",
        nargs="+",
    ...
    parser.add_argument(
        "--csv-output", type=str, default="ocr_eval_results.csv", help="Output CSV file for detailed results"
    )
    ...
    try:
                ...
                for output_type in args.output_types:
                    for input_type in args.input_types or [None]:
                        ...
                        num_questions = (
                            ...
                            if task_type in ["conditional_detection", "localized_reading"]
                            else 1
                        )
                        ...
                        for question_idx in range(num_questions):
                            try:
                                ...
                                deterministic_seed = hash(
                                    f"{entry.filename}_{task_type}_{input_type}_{output_type}_{question_idx}"
                                ) % (2**31)
                                ...
                                if output_type == "box" and (
                                    not expected_answer or expected_answer == [] or expected_answer == "[]"
                                ):
                                    logger.debug(
                                        f"Skipping {task_type}/{output_type} task {question_idx} for {entry.filename}: empty box answer"
                                    )
                                    continue

                                if "{box}" in question or "{text}" in question:
                                    logger.debug(
                                        f"Skipping {task_type}/{output_type} task {question_idx} for {entry.filename}: placeholder in question"
                                    )
                                    continue
                                ...
                                logger.warning(
                                    f"Failed to generate {task_type}/{output_type} task {question_idx} for {entry.filename}: {e}"
                                )
        ...
        for gpu_id in range(num_workers):
            if gpu_id < len(task_chunks) and len(task_chunks[gpu_id]) > 0:
                p = mp.Process(target=worker_process, args=(gpu_id, task_chunks[gpu_id], args, return_dict))
                p.start()
                processes.append(p)
