...
from utils.text import calculate_anls, calculate_character_error_rate, calculate_word_error_rate
...
def compute_text_metrics(
    expected_clean: str, predicted_clean: str, tokenizer=None, task_hint: str = "", handle_cjk_wer: bool = True
) -> dict:
    ...
    if (
        handle_cjk_wer
        and _looks_cjk(expected_clean)
        and _looks_cjk(predicted_clean)
    ...
    return {"character_error_rate": cer, "word_error_rate": wer, "anls": anls}
...
def score_text_reading_predictions(
    ...
    def compute_metrics_row(row):
        ...
        try:
            return compute_text_metrics(
                row["expected_clean"], row["predicted_clean"], tokenizer=None, task_hint=csv_path, handle_cjk_wer=True
            )
        except Exception as e:
            print(f"Warning: Failed to compute metrics for row {row.name}: {e}", flush=True)
            return {"character_error_rate": 1.0, "word_error_rate": float("nan"), "anls": 0.0}
    ...
    if len(em):
        print(
            f"Exact matches: {int(pd.to_numeric(em, errors='coerce').fillna(0).sum())} "
...
def main():
    ...
    parser.add_argument("--expected-col", default="expected_answer", help="Column name containing expected text")
    parser.add_argument(
        "--predicted-col", default="predicted_answer", help="Column name containing predicted text/JSON"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--normalize-whitespace", action="store_true", help="Force whitespace normalization (auto-detected by default)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-unicode-norm", action="store_true", help="Force NO Unicode normalization (auto-detected by default)"
    )
