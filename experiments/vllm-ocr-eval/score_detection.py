...
def _get_default_metrics(iou_metric_columns: list[str]) -> dict[str, Any]:
    ...
    default_metrics = {
        ...
        "mean_recall": 0.0,
        "mean_f1": 0.0,
        "mean_expected_area": 0.0,
        "mean_predicted_area": 0.0,
    }
...
def assign_matches(
    expected_boxes: list[list[int]], predicted_boxes: list[list[int]], iou_threshold: float = 0.5
...
def compute_detection_metrics(
    expected_boxes: list[list[int]], predicted_boxes: list[list[int]], iou_thresholds: list[float] = None
) -> dict[str, Any]:
    ...
    if E == 0 or P == 0:
        ...
        for iou_thresh in iou_thresholds:
            ...
            if E == 0 and P == 0:
                ...
                metrics.update(
                    {
                        f"tp_iou_{thresh_str}": 0,
                        f"fp_iou_{thresh_str}": 0,
                        f"fn_iou_{thresh_str}": 0,
                        f"precision_iou_{thresh_str}": 1.0,
                        f"recall_iou_{thresh_str}": 1.0,
                        f"f1_iou_{thresh_str}": 1.0,
                ...
                metrics.update(
                    {
                        f"tp_iou_{thresh_str}": 0,
                        f"fp_iou_{thresh_str}": P,
                        f"fn_iou_{thresh_str}": 0,
                        f"precision_iou_{thresh_str}": 0.0,
                        f"recall_iou_{thresh_str}": 1.0,
                        f"f1_iou_{thresh_str}": 0.0,
                        f"mean_matched_iou_{thresh_str}": 0.0,
                    }
                )
            else:  # P == 0
                ...
                metrics.update(
                    {
                        f"tp_iou_{thresh_str}": 0,
                        f"fp_iou_{thresh_str}": 0,
                        f"fn_iou_{thresh_str}": E,
                        f"precision_iou_{thresh_str}": 1.0,
                        f"recall_iou_{thresh_str}": 0.0,
                        f"f1_iou_{thresh_str}": 0.0,
                        f"mean_matched_iou_{thresh_str}": 0.0,
                    }
                )
        ...
        if E == 0 and P == 0:
            ...
            metrics.update({"mean_precision": 1.0, "mean_recall": 1.0, "mean_f1": 1.0, "matched_ious": []})
        elif E == 0:
            # Expected empty, predicted non-empty
            metrics.update({"mean_precision": 0.0, "mean_recall": 1.0, "mean_f1": 0.0, "matched_ious": []})
        else:  # P == 0
            # Expected non-empty, predicted empty
            metrics.update({"mean_precision": 1.0, "mean_recall": 0.0, "mean_f1": 0.0, "matched_ious": []})
        return metrics
...
def main():
    ...
    parser.add_argument(
        "--expected-col", default="expected_answer", help="Column name containing expected boxes (JSON)"
    )
    parser.add_argument(
        "--predicted-col", default="predicted_answer", help="Column name containing predicted boxes (JSON)"
    )
    parser.add_argument(
        "--question-col", default="question", help="Column name containing question text (default: question)"
    )
    parser.add_argument("--iou-step", type=float, default=0.05, help="Step size for IoU thresholds (default: 0.05)")
    parser.add_argument("--iou-min", type=float, default=0.05, help="Minimum IoU threshold (default: 0.05)")
    parser.add_argument("--iou-max", type=float, default=0.95, help="Maximum IoU threshold (default: 0.95)")
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing metric columns")
    ...
    print(
        f"Using {len(iou_thresholds)} IoU thresholds: {iou_thresholds[0]} to {iou_thresholds[-1]} (step: {args.iou_step})"
    )
