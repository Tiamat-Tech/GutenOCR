    ...
    def to_prompt_builder_format(self) -> dict[str, Any]:
        ...
        return {
            "text": {
                ...
                "text2d": self.text2d,
                "lines": [
                    {"text": line.get("text", ""), "box": line.get("box", line.get("bbox", []))} for line in self.lines
                ],
                "words": [
                    {"text": word.get("text", ""), "box": word.get("box", word.get("bbox", []))} for word in self.words
                ],
                "paragraphs": [
                    {"text": para.get("text", ""), "box": para.get("box", para.get("bbox", []))}
                    for para in self.paragraphs
                ],
                "latex": [
                    {"text": item.get("text", ""), "box": item.get("box", item.get("bbox", []))} for item in self.latex
                ],
            },
            "image": {"width": self.image_width, "height": self.image_height, "path": self.image_path},
...
class TarShardDataset:
    ...
    def get_entries(self, limit: int | None = None) -> list[OCREntry]:
