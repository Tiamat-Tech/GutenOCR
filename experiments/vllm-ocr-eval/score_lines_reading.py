...
from utils.parse import normalize_structured_keys, parse_json_output
...
def compute_lines_metrics(expected_output, predicted_output, tokenizer) -> dict:
    ...
    normalized_expected = normalize_structured_keys(
    ...
    text_metrics = calculate_comprehensive_text_metrics(combined_expected_text, combined_predicted_text, tokenizer)
    ...
    if (
        ...
        if cost_matrix.size > 0:
            ...
            matched_pairs = [(i, j, iou_matrix[i, j]) for i, j in zip(row_ind, col_ind) if iou_matrix[i, j] >= 0.5]
...
def mark_template_placeholders(df: pd.DataFrame, question_col: str = "question") -> pd.DataFrame:
    ...
    if question_col not in df.columns:
        print(
            f"Warning: Question column '{question_col}' not found; skipping template placeholder marking.", flush=True
        )
    ...
    if template_count > 0:
        print(
            f"Detected {template_count} samples with template placeholders ({{text}}) in '{question_col}'. "
            f"These will be excluded from metric computation but kept in the CSV.",
            flush=True,
        )
    else:
...
def score_lines_reading_predictions(
    ...
    metric_columns = [
        ...
        "overall_token_error_rate",
        "overall_anls",
        "bbox_precision",
        "bbox_recall",
        "bbox_f1",
        "bbox_mean_iou",
        "bbox_total_predicted",
        "matched_exact_match",
        "matched_character_error_rate",
        "matched_token_error_rate",
        "matched_anls",
        "matched_box_count",
    ...
    print(
        f"Computing lines reading evaluation metrics for {total_rows} samples (excluding placeholders)...", flush=True
    )
    ...
    valid_df = scored_df[scored_df["valid_json"]].copy() if len(scored_df) > 0 else pd.DataFrame()
    if len(valid_df) > 0:
        print(f"Valid samples for metrics: {len(valid_df)}")

        print("\n📝 OVERALL TEXT METRICS:")
        print(
            f"  Exact matches: {valid_df['overall_exact_match'].sum():.0f} ({100 * valid_df['overall_exact_match'].mean():.1f}%)"
        )
...
def main():
    ...
    parser.add_argument(
        "--tokenizer", default="Qwen/Qwen2.5-VL-3B-Instruct", help="Tokenizer name/path for token-level metrics"
    )
    parser.add_argument(
        "--expected-col", default="expected_answer", help="Column name containing expected structured output"
    )
    parser.add_argument(
        "--predicted-col", default="predicted_answer", help="Column name containing predicted structured output/JSON"
    )
    parser.add_argument(
